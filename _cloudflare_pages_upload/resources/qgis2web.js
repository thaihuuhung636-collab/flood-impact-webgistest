...
var map = new ol.Map({
    ...
    layers: layersList,
    view: new ol.View({
         maxZoom: 28, minZoom: 0
    })
});

//initial view - epsg:3857 coordinates if not "Match project CRS"
map.getView().fit([11965072.679072, 1861034.878614, 11975989.985970, 1875249.277764], map.getSize());
...
function updatePopup() {
    if (typeof window.FLOOD_CLICK_OBJECT_ENABLED === 'function' && !window.FLOOD_CLICK_OBJECT_ENABLED()) {
        popupContent = '';
        container.style.display = 'none';
        closer.blur();
        stopMediaInPopup();
        if (typeof window.floodHideAttributePanel === 'function') window.floodHideAttributePanel();
        return;
    }
    if (popupContent) {
        if (typeof window.floodShowQgis2WebAttributePanel === 'function') {
            window.floodShowQgis2WebAttributePanel(popupContent);
            content.innerHTML = '';
            container.style.display = 'none';
            return;
        }
